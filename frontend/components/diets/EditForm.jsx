...
function EditForm({id}) {
  ...
  const onSubmit = async (e) => {
    ...
    const res = await axios({
      method: "POST",
      url: `${DIET.DIETSLIST}/${id}`,
      mode: "cors",
      headers: authHeader(),
      data: formData,
    }).then(() => {
      router.push(`/foodRecord/detail/${id}`);
      alert("식단 기록 수정 성공");
    });
    console.log(res);
  };
        ...
        <div className="grid grid-cols-4">
          {vegetypes.map((item) => (
            <label key={item.id} className="pt-1">
              ...
              />
              <div className="w-14 h-14 pt-1 text-xs mr-2 md:w-16 md:h-16 md:pt-2 md:mx-2 rounded-full border border-gray-3 text-center hover:bg-main   peer-checked:bg-main">
                <Image
